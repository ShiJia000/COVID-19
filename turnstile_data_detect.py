...
# Distinct Date & Time
distinct_date = spark.sql("SELECT `date`, `time` FROM turnstile GROUP BY `date`, `time` ORDER BY `date`, `time`")
distinct_date.coalesce(1).rdd.map(lambda x: x[0] + ',' + x[1]).saveAsTextFile("distinct-date.out")
