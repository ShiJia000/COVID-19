...
check_rate.coalesce(1).rdd.map(lambda x: ((x[0],x[1]),(x[2],x[3],[4]))).saveAsTextFile("illegal_rate.out")
#-- rate is null or not a number
check_rate2 = spark.sql("SELECT `zcta_cum.perc_pos` FROM covid ORDER BY `zcta_cum.perc_pos` DESC")
check_rate2.coalesce(1).rdd.map(lambda x: (x[0])).saveAsTextFile("illegal_rate2.out")
#-- positive num < total num
check_rate3 = spark.sql("SELECT * FROM covid WHERE `Positive` > `Total`")
check_rate3.coalesce(1).rdd.map(lambda x: ((x[0],x[1]),(x[2],x[3]))).saveAsTextFile("illegal_rate3.out")
